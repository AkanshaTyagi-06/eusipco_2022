...
mcca = MCCA(n_components=n_com,i_mcca_method='auto',regs=0.5)
mcca.fit(train_data)
print("eigen values",mcca.evals_)
...
for i in range(len(views_list)):
    ...
    for j in range(len(views_list)):
        print("s e",s,e)
        mat[:,s:e] = mcca.transform_view(train_data[i],j)
        s = e
        e = e + n_com
...
for i in range(len(views_list)):
    ...
    for j in range(len(views_list)):
        print("s e",s,e)
        mat[:,s:e] = mcca.transform_view(test_data[i],j)
        s = e
        e = e + n_com
